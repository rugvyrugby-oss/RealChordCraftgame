...
const GROOVE_PATTERNS = {
  ...
  cinematic: {
    swing: 0, humanize: 0.01,
    hits: [
      { t: 0.0, notes: "roll", vel: 0.55, len: 1.6 },
    ],
  },
  ambient: {
    swing: 0, humanize: 0.008,
    hits: [
      { t: 0.0, notes: "roll", vel: 0.45, len: 2.0 },
    ],
  },
  house: {
    swing: 0, humanize: 0.006,
    hits: [
...
const BLACK_POSITIONS = { 0:"C#3",1:"D#3",3:"F#3",4:"G#3",5:"A#3",7:"C#4",8:"D#4",10:"F#4",11:"G#4",12:"A#4" };
...
const FLOAT_NOTES = [
  { s:"♩", top:"8%",  left:"5%",  sz:48, a:"floatNote1 9s ease-in-out infinite" },
  { s:"♪", top:"15%", left:"88%", sz:36, a:"floatNote2 11s ease-in-out infinite 1.2s" },
  { s:"♫", top:"35%", left:"93%", sz:52, a:"floatNote3 13s ease-in-out infinite 2.5s" },
  { s:"♬", top:"60%", left:"3%",  sz:44, a:"floatNote1 12s ease-in-out infinite 3.5s" },
  { s:"♩", top:"75%", left:"91%", sz:32, a:"floatNote2 10s ease-in-out infinite 1.8s" },
  { s:"♪", top:"85%", left:"8%",  sz:40, a:"floatNote3 11.5s ease-in-out infinite 4.2s" },
  { s:"♫", top:"50%", left:"96%", sz:28, a:"floatNote1 14s ease-in-out infinite .7s" },
  { s:"♬", top:"25%", left:"2%",  sz:56, a:"floatNote2 9.5s ease-in-out infinite 2.8s" },
  { s:"♩", top:"92%", left:"50%", sz:34, a:"floatNote3 12.5s ease-in-out infinite 1.5s" },
  { s:"♪", top:"5%",  left:"50%", sz:30, a:"floatNote1 10.5s ease-in-out infinite 3.8s" },
  { s:"♬", top:"45%", left:"15%", sz:38, a:"floatNote2 13.5s ease-in-out infinite 2.2s" },
  { s:"♫", top:"68%", left:"78%", sz:42, a:"floatNote3 10.8s ease-in-out infinite 5.1s" },
  { s:"♩", top:"20%", left:"70%", sz:30, a:"floatNote1 11.8s ease-in-out infinite 0.4s" },
  { s:"♪", top:"55%", left:"40%", sz:26, a:"floatNote2 12.8s ease-in-out infinite 6s" },
  { s:"♬", top:"38%", left:"55%", sz:34, a:"floatNote3 14.5s ease-in-out infinite 3.1s" },
];
...
const THEMES = {
  midnight: {
    ...
    swatch: "#6366f1",
    bg: "#000000",
    ambient: "radial-gradient(ellipse 60% 40% at 15% 10%, rgba(50,15,100,0.04) 0%, transparent 50%), radial-gradient(ellipse 45% 35% at 85% 85%, rgba(8,50,75,0.035) 0%, transparent 50%)",
    accent: "#6366f1",
    accent2: "#a78bfa",
  ...
  velvet: {
    ...
    swatch: "#e0729c",
    bg: "#0f0508",
    ambient: "radial-gradient(ellipse 65% 45% at 20% 12%, rgba(190,40,90,0.07) 0%, transparent 52%), radial-gradient(ellipse 50% 40% at 82% 84%, rgba(120,30,80,0.06) 0%, transparent 52%)",
    accent: "#e0729c",
    accent2: "#f0a8c0",
  ...
  forest: {
    ...
    swatch: "#34d399",
    bg: "#03100b",
    ambient: "radial-gradient(ellipse 65% 45% at 18% 12%, rgba(20,120,70,0.07) 0%, transparent 52%), radial-gradient(ellipse 50% 40% at 84% 86%, rgba(15,90,90,0.06) 0%, transparent 52%)",
    accent: "#34d399",
    accent2: "#6ee7b7",
  ...
  sunset: {
    ...
    swatch: "#fb923c",
    bg: "#120803",
    ambient: "radial-gradient(ellipse 65% 45% at 18% 12%, rgba(200,90,30,0.08) 0%, transparent 52%), radial-gradient(ellipse 52% 40% at 84% 84%, rgba(170,50,60,0.06) 0%, transparent 52%)",
    accent: "#fb923c",
    accent2: "#fdba74",
  ...
  mono: {
    ...
    swatch: "#a3a3a3",
    bg: "#0a0a0a",
    ambient: "radial-gradient(ellipse 60% 40% at 15% 10%, rgba(120,120,120,0.05) 0%, transparent 50%), radial-gradient(ellipse 45% 35% at 85% 85%, rgba(80,80,80,0.04) 0%, transparent 50%)",
    accent: "#d4d4d4",
    accent2: "#fafafa",
...
const EXAMPLE_PROGRESSION = {
  ...
  chords: [
    { name: "Dm9", duration: 1, notes: ["D2", "F3", "A3", "C4", "E4"], function: "subdominant", romanNumeral: "ii" },
    { name: "G13", duration: 1, notes: ["G2", "B3", "F4", "A4", "E5"], function: "dominant", romanNumeral: "V" },
    { name: "Cmaj7", duration: 1, notes: ["C3", "E4", "G4", "B4"], function: "tonic", romanNumeral: "I" },
    { name: "Am11", duration: 1, notes: ["A2", "G3", "C4", "E4", "D5"], function: "tonic", romanNumeral: "vi" },
  ],
};
...
export default function ChordApp() {
  ...
  const [sampleStatus, setSampleStatus] = useState("⏳ Loading real piano samples...");
  ...
  const [proSubhead, setProSubhead] = useState("Pro launches soon. Be the first to know.");
  const [shareUrl, setShareUrl] = useState("");
  const [usageLeft, setUsageLeft] = useState(DAILY_LIMIT - getUsage());

  // Settings (persisted to localStorage)
  const [themeId, setThemeId] = useState(() => {
    try { return localStorage.getItem("theme") || "midnight"; } catch { return "midnight"; }
  });
  const [showFloatingNotes, setShowFloatingNotes] = useState(() => {
    try { return localStorage.getItem("floatingNotes") !== "off"; } catch { return true; }
  });
  const [reduceMotion, setReduceMotion] = useState(() => {
    try { return localStorage.getItem("reduceMotion") === "on"; } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    const reverbNode = new Tone.Reverb({ decay: 2.5, wet: 1, preDelay: 0.01 }).toDestination();
    ...
    const rhodes = new Tone.PolySynth(Tone.FMSynth, {
      ...
      modulationEnvelope: { attack: 0.02, decay: 0.2, sustain: 0.3, release: 1 },
    }).toDestination();
    ...
    return () => {
      [piano, rhodes, pad, pluck, reverbNode].forEach((n) => { try { n.dispose(); } catch {} });
    };
  }, []);
  ...
  useEffect(() => {
    const loadFromHash = () => {
      ...
      try {
        ...
        setResult({
          title: p.t, key: p.k, bpm: p.b, timeSignature: p.ts,
          genre: p.g, vibe: p.v, theory: p.th,
          chords: p.c.map((c) => ({
            name: c.n, duration: c.d, notes: c.no, function: c.f, romanNumeral: c.r,
          })),
        });
        if (p.b) setBpm(p.b);
  ...
  const performChord = useCallback((notes, chordDurSec, tone, groove) => {
    const Tone = window.Tone;
    if (!Tone) { console.warn("performChord: Tone not available"); return; }
    const sampler = getSampler(tone);
    if (!sampler) { console.warn("performChord: no sampler"); return; }
    ...
    if (reverbRef.current && reverbRef.current.loaded !== false) {
      try {
        ...
        if (reverbAmount > 0.01) {
          try { sampler.disconnect(reverbRef.current); } catch {}
          try { sampler.connect(reverbRef.current); } catch {}
        }
      } catch (e) { /* dry fallback */ }
    }
    ...
    if (arpAmount > 0.02) {
      ...
      if (arpAmount < 0.7 && groove.hits[0]) {
        activeGroove.hits.unshift({ t: 0, notes: "low", vel: groove.hits[0].vel * (1 - arpAmount), len: groove.hits[0].len });
      }
    }
    ...
    activeGroove.hits.forEach((hit) => {
      ...
      if (isOffbeat && activeGroove.swing) hitTime += activeGroove.swing * 0.25;
      const baseDelayMs = hitTime * chordDurSec * 1000;
      let hitNotes;
      if (hit.notes === "arp") hitNotes = [activeGroove._arpNotes[hit.arpIdx]];
      ...
      hitNotes.forEach((n, i) => {
        ...
        const humanDrift = (Math.random() - 0.5) * hum * 1000;
        const delay = Math.max(0, baseDelayMs + rollSpread + humanDrift);
        const vel = Math.min(1, Math.max(0.1, hit.vel * (0.85 + Math.random() * 0.3)));
        firedCount++;
        setTimeout(() => {
          try { sampler.triggerAttackRelease(n, noteDur, undefined, vel); }
          catch (e) { console.warn("triggerAttackRelease failed for", n, e); }
        }, delay);
      });
    });
    console.log(`▶ performChord: scheduled ${firedCount} notes for chord [${notes.join(",")}] with tone=${tone} arp=${arpeggiate}`);
  }, [reverb, decay, getSampler, arpeggiate]);

  // Simple single-chord preview (used by tap-to-preview) — gentle roll
  const playChordTone = useCallback((notes, durationSec, tone) => {
    const Tone = window.Tone;
    if (!Tone) return;
    const sampler = getSampler(tone);
    if (!sampler) return;
    ...
    notes.forEach((n, i) => {
      ...
      setTimeout(() => {
        try { sampler.triggerAttackRelease(n, durationSec, undefined, vel); }
        catch (e) { console.warn("Note failed:", n, e); }
  ...
  const checkUsage = () => {
    ...
    try { localStorage.setItem(todayKey(), used + 1); } catch {}
  ...
  const generate = async () => {
    ...
    if (!checkUsage()) { setShowLimit(true); return; }
  ...
  const fetchVariations = async () => {
    ...
    try {
      const promptForAI = lastPrompt || `${result.vibe || ""} ${result.genre || ""}`.trim() || result.title;
      const currentChords = result.chords.map(c => c.name).join(" - ");
      const rejectedNote = rejected.length ? `Avoid these chord names entirely: ${rejected.join(", ")}.` : "";
...
Each variation MUST feel meaningfully different from the others — not just inversions. Vary modes, borrowed chords, substitutions, tritone subs, modal interchange. Keep the same vibe & tempo range. The user's current progression was: ${currentChords}. Make these new ones genuinely fresh alternatives. ${rejectedNote} Use rich voicings with 7ths/9ths/11ths, root low, color tones high.`;
  ...
  const fetchMutations = async (slotIdx) => {
    ...
    try {
      ...
      const context = result.chords.map((c, i) => i === slotIdx ? "[?]" : c.name).join(" - ");
...
Key: ${result.key}. Surrounding chords: ${context} — the [?] slot is what you're replacing. Original chord there was ${chord.name}. Give 3 genuinely different musical choices — try modal interchange, secondary dominants, tritone subs, chromatic neighbors. Use rich voicings. ${rejectedNote}`;
  ...
  const handlePlay = async () => {
    console.log("▶ handlePlay clicked. result:", !!result, "isPlaying:", isPlaying);
    if (!result) { console.warn("No result to play"); return; }
    if (isPlaying) { stopLoopRef.current = true; return; }
    setLooping(false);
    setIsPlaying(true);
  ...
  const exportAudio = async () => {
    ...
    try {
      ...
      const loops = 2;
      const totalSec =
        result.chords.reduce((a, c) => a + (c.duration || 1), 0) * beatDur * loops + 4;
      const rw = (reverb / 100) * 0.5;

      const buffer = await Tone.Offline(async () => {
        const offReverb = new Tone.Reverb({ decay: 4, wet: rw }).toDestination();
        ...
        const splitV = (notes) => {
          ...
          const sorted = [...notes];
          const splitIdx = Math.max(1, Math.floor(sorted.length / 2.5));
          return { low: sorted.slice(0, splitIdx), high: sorted.slice(splitIdx), all: sorted };
        };
          ...
          result.chords.forEach((chord) => {
            ...
            groove.hits.forEach((hit) => {
              ...
              hitNotes.forEach((n, ni) => {
                const rollSpread = isRoll ? ni * (0.018 + Math.random() * 0.014) : 0;
                const humanDrift = (Math.random() - 0.5) * (groove.humanize || 0.012);
                const vel = Math.min(1, Math.max(0.1, hit.vel * (0.85 + Math.random() * 0.3)));
                instr.triggerAttackRelease(
                  n, noteDur, Math.max(t, baseTime + rollSpread + humanDrift), vel
                );
              });
            });
            t += chordDurSec;
      ...
      a.href = url;
      a.download = `${(result.title || "chord_progression").replace(/\s+/g, "_")}.wav`;
      a.click();
    } catch (e) {
  ...
  const exportMidi = () => {
    ...
    try {
      ...
      a.download = `${(result.title || "progression").replace(/\s+/g, "_")}.mid`;
      a.click();
    } catch (e) {
  ...
  const buildShareUrl = () => {
    ...
    const payload = {
      t: result.title, k: result.key, b: bpm, ts: result.timeSignature,
      g: result.genre, v: result.vibe, th: result.theory,
      c: result.chords.map((c) => ({
        n: c.name, d: c.duration, no: c.notes, f: c.function, r: c.romanNumeral,
      })),
      tn: activeTone, rv: reverb, sn: decay,
    };
  ...
  const shareToPlatform = (platform) => {
    ...
    else if (platform === "reddit") shareURL = `https://reddit.com/submit?url=${url}&title=${title}`;
  ...
  const toggleFloatingNotes = () => {
    setShowFloatingNotes((v) => {
      const next = !v;
      try { localStorage.setItem("floatingNotes", next ? "on" : "off"); } catch {}
      return next;
    });
  };
  const toggleReduceMotion = () => {
    setReduceMotion((v) => {
      const next = !v;
      try { localStorage.setItem("reduceMotion", next ? "on" : "off"); } catch {}
      return next;
    });
  };
  ...
  const dismissWelcome = () => {
    try { localStorage.setItem("welcome_shown_v1", "1"); } catch {}
  ...
  const previewProgression = async (progression) => {
    ...
    for (let i = 0; i < progression.chords.length; i++) {
      ...
      await new Promise((r) => setTimeout(r, beatDur * (c.duration || 1) * 1000));
    }
    setHighlightedNotes(new Set());
  };
  ...
  return (
    <div style={themedBody}>
        ...
        <div style={S.notesBg}>
          {FLOAT_NOTES.map((n, i) => (
            <div key={i} style={{
              ...
              fontSize: n.sz, color: theme.note, userSelect: "none",
      ...
      {/* ── Welcome Modal ── */}
      {showWelcome && (
        <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && dismissWelcome()}>
          <div style={{ ...S.modalBox, maxWidth: 520, borderColor: "rgba(99,102,241,0.4)" }}>
            <button style={S.modalClose} onClick={dismissWelcome}>×</button>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>🎹</div>
              <div style={S.kicker}>Welcome to</div>
              <h2 style={S.modalTitle}>The Ultimate Chord Engine</h2>
              <p style={{ color: "#94a3b8", fontSize: 14, lineHeight: 1.6 }}>
                Describe any feeling — get a real piano progression instantly. Free forever.
              </p>
            </div>
            <div style={S.featureBox}>
              <div style={S.featureLabel}>FREE PLAN INCLUDES</div>
              <div style={S.featureList}>
                ✓ 4-chord AI progressions<br />
                ✓ Real grand piano playback<br />
                ✓ Loop + share + MIDI export<br />
                ✓ 10 generations per day
              </div>
            </div>
            <div style={{ ...S.featureBox, background: "linear-gradient(135deg, rgba(99,102,241,0.12) 0%, rgba(167,139,250,0.08) 100%)", borderColor: "rgba(99,102,241,0.3)" }}>
              <div style={{ ...S.featureLabel, color: "#a78bfa" }}>✨ PRO — from $12/mo or $100 lifetime</div>
              <div style={S.featureList}>
                ✦ 8 & 16-bar progressions<br />
                ✦ All instrument tones (Rhodes, Pad, Pluck)<br />
                ✦ Unlimited generations<br />
                ✦ Bass line + melody layer<br />
                ✦ Save & revisit your projects<br />
                ✦ Export individual stems (chords, bass, melody)<br />
                ✦ Exclusive monthly MIDI packs<br />
                ✦ Studio-quality WAV (24-bit, no watermark)
              </div>
            </div>
            <EmailCapture source="welcome" accent="#6366f1" />
            <button style={S.ghostBtn} onClick={dismissWelcome}>Start exploring →</button>
          </div>
        </div>
      )}
      ...
      {/* ── Pro Modal ── */}
      {showPro && (
        <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && setShowPro(false)}>
          <div style={{ ...S.modalBox, maxWidth: 560, borderColor: "rgba(167,139,250,0.4)" }}>
            <button style={S.modalClose} onClick={() => setShowPro(false)}>×</button>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              <div style={{ fontSize: 40, marginBottom: 8 }}>✨</div>
              <div style={{ ...S.kicker, color: "#a78bfa" }}>Upgrade to Pro</div>
              <h3 style={S.modalTitle}>{proHeadline}</h3>
              <p style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.6 }}>{proSubhead}</p>
            </div>

            {/* What's included */}
            <div style={{ ...S.featureBox, background: "rgba(167,139,250,0.06)", borderColor: "rgba(167,139,250,0.2)", marginBottom: 20 }}>
              <div style={{ ...S.featureLabel, color: "#a78bfa", marginBottom: 10 }}>EVERYTHING IN PRO</div>
              <div style={S.featureList}>
                ✦ All 4 instrument tones unlocked<br />
                ✦ 8-bar & 16-bar progressions<br />
                ✦ Unlimited daily generations<br />
                ✦ Bass + melody layers<br />
                ✦ Save & revisit your projects<br />
                ✦ Export individual stems (chords, bass, melody)<br />
                ✦ Exclusive monthly MIDI packs<br />
                ✦ 24-bit studio WAV exports
              </div>
            </div>

            {/* Pricing plans */}
            <div style={{ ...S.featureLabel, color: "#a78bfa", marginBottom: 12, textAlign: "center" }}>CHOOSE YOUR PLAN</div>
            <div style={S.pricingGrid} className="pricing-grid-responsive">
              {PRICING_PLANS.map((plan) => (
                ...
                >
                  ...
                  {plan.badge && (
                    <div style={{ ...S.bestValueBadge, background: "#f59e0b", color: "#0a0a0f" }}>
                      {plan.badge}
                    </div>
                  )}
                  ...
                  <div style={S.priceAmount}>
                    <span style={{ fontSize: 14, color: "#94a3b8", verticalAlign: "top", marginRight: 2 }}>$</span>
                    {plan.price}
                    <span style={{ fontSize: 13, color: "#94a3b8", fontWeight: 400, marginLeft: 4 }}>
                      {plan.period}
                    </span>
                  </div>
                  {plan.note && (
                    <div style={S.priceNote}>{plan.note}</div>
                  )}
                  <button
                    style={{
                      ...S.priceCta,
                    ...
                    onClick={() => {
                      if (plan.checkoutUrl) {
                        ...
                        alert("Checkout launching soon! Drop your email below and we'll notify you the moment Pro goes live.");
                      }
                    }}
                  >
                    {plan.cta}
            ...
            <div style={{ fontSize: 11, color: "#475569", textAlign: "center", marginTop: 16, marginBottom: 18, lineHeight: 1.5 }}>
              Secure checkout via Stripe · Cancel anytime · 7-day money-back guarantee
            </div>

            <EmailCapture source="pro_modal" accent="linear-gradient(135deg, #6366f1 0%, #a78bfa 100%)" />
          </div>
        </div>
      )}
      ...
      {/* ── Daily Limit Modal ── */}
      {showLimit && (
        <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && setShowLimit(false)}>
          <div style={{ ...S.modalBox, maxWidth: 440, borderColor: "rgba(245,158,11,0.4)" }}>
            <button style={S.modalClose} onClick={() => setShowLimit(false)}>×</button>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, marginBottom: 10 }}>⏳</div>
              <div style={{ ...S.kicker, color: "#f59e0b" }}>Daily limit reached</div>
              <h3 style={S.modalTitle}>You've used your 10 free generations today</h3>
              ...
              <button
                style={{ ...S.primaryBtn, width: "100%", background: "linear-gradient(135deg, #6366f1 0%, #a78bfa 100%)" }}
                onClick={() => { setShowLimit(false); openProModal("Unlimited generations", "Pro gives you unlimited daily generations and more."); }}
              >✨ Get notified about Pro</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* ── Share Modal ── */}
      {showShare && (
        <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && setShowShare(false)}>
          <div style={{ ...S.modalBox, maxWidth: 480, borderColor: "rgba(99,102,241,0.3)" }}>
            <button style={S.modalClose} onClick={() => setShowShare(false)}>×</button>
            <div style={S.kicker}>Share Progression</div>
            <h3 style={{ ...S.modalTitle, marginBottom: 20 }}>"{result?.title}"</h3>
            <div style={S.urlBox}>{shareUrl}</div>
            <button style={{ ...S.primaryBtn, width: "100%", marginBottom: 12 }} onClick={copyShareUrl}>
              📋 Copy Link
            </button>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
              <button style={S.platformBtn} onClick={() => shareToPlatform("twitter")}>𝕏 Twitter</button>
              <button style={S.platformBtn} onClick={() => shareToPlatform("facebook")}>f Facebook</button>
              <button style={S.platformBtn} onClick={() => shareToPlatform("reddit")}>↗ Reddit</button>
            </div>
      ...
      {/* ── Settings Modal ── */}
      {showSettings && (
        <div style={S.modalOverlay} onClick={(e) => e.target === e.currentTarget && setShowSettings(false)}>
          <div style={{ ...S.modalBox, maxWidth: 460, borderColor: theme.panelBorder }}>
            <button style={S.modalClose} onClick={() => setShowSettings(false)}>×</button>
            <div style={{ ...S.kicker, color: theme.accent }}>⚙ Settings</div>
            <h3 style={{ ...S.modalTitle, marginBottom: 24 }}>Customize your engine</h3>

            {/* Theme picker */}
            <div style={{ marginBottom: 24 }}>
              <div style={{ ...S.featureLabel, color: theme.accent, marginBottom: 12 }}>THEME</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8 }}>
                {Object.entries(THEMES).map(([id, t]) => (
                  <button
                    ...
                    style={{
                      background: t.bg,
                      border: themeId === id ? `2px solid ${t.swatch}` : "2px solid rgba(255,255,255,0.1)",
                      borderRadius: 10,
                  ...
                  >
                    <div style={{
                      ...
                      boxShadow: themeId === id ? `0 0 10px ${t.swatch}` : "none",
                    }} />
                    <span style={{
            ...
            {/* Toggles */}
            <div style={{ ...S.featureLabel, color: theme.accent, marginBottom: 12 }}>DISPLAY</div>
            <SettingsToggle
            ...
            <div style={{ fontSize: 11, color: "#475569", marginTop: 20, textAlign: "center", lineHeight: 1.5 }}>
      ...
      <div style={S.app}>
        ...
        <div style={S.topBar}>
          ...
          <button style={S.proBtn} onClick={() => openProModal("Unlock everything", "Pro launches soon. Be the first to know.")}>
            ✨ Pro
          </button>
        </div>
        ...
        <h1 style={S.h1}>
          ...
          <span style={{ color: "#818cf8", fontStyle: "italic" }}>Hear the chords.</span>
        </h1>
        <p style={S.subtitle}>
          Type any mood, tempo, or genre — get a full progression with real piano playback.
        </p>
        {sampleStatus && (
          <div style={{ ...S.sampleStatus, color: samplesReady ? "#10b981" : "#475569" }}>
            {sampleStatus}
          </div>
        )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 36 }}>
          {SUGGESTIONS.map((s, i) => (
            <button key={i} className="suggestion" onClick={() => setPrompt(s)}>{s}</button>
          ))}
        </div>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "60px 0", color: "#6366f1" }}>
            <div className="note-spin" style={{ fontSize: 36, marginBottom: 16 }}>♩</div>
            <div style={{ fontSize: 13, color: "#64748b", fontFamily: "monospace", letterSpacing: 2 }}>
              COMPOSING...
            </div>
          </div>
        )}
          ...
          <div className="fade-in">
            ...
            {isExample && (
              <div style={S.exampleBanner}>
                ...
                <span>
                  This is an <strong style={{ color: theme.accent2 }}>example</strong> — hit{" "}
                  <strong style={{ color: theme.textBright }}>▶ Play</strong> to hear it, or describe your own vibe above.
                </span>
              </div>
            )}
            <div style={S.resultLabel}>{isExample ? "Example Progression" : "Generated Progression"}</div>
            <div style={S.resultTitle}>{result.title}</div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 24, alignItems: "center" }}>
              {[`♩ ${result.bpm} BPM`, result.key, result.timeSignature, result.genre].map((t, i) => (
                <span key={i} style={S.metaTag}>{t}</span>
              ))}
              {result.style && (
                <span style={S.grooveBadge}>
                  ♫ {result.style} groove
                </span>
              )}
            </div>
            ...
            <div style={S.pianoWrap}>
              <canvas ref={canvasRef} width={720} height={120} style={{ display: "block", margin: "0 auto" }} />
            </div>

            {/* Chord grid */}
            <div style={S.chordGrid} className="chord-grid-responsive">
              {result.chords.map((chord, i) => {
                const col = functionColors[chord.function] || functionColors.other;
                ...
                return (
                  <div
                    ...
                    style={{
                      background: isActive ? `rgba(${col.rgb},.18)` : "rgba(255,255,255,.04)",
                      borderColor: isMutating ? theme.accent2 : (isActive ? col.hex : "rgba(255,255,255,.08)"),
                      position: "relative",
                    }}
                  >
                    {/* Mini action buttons in top corners */}
                    <button
                      onClick={(e) => { e.stopPropagation(); fetchMutations(i); }}
                      title="Try alternatives for this chord"
                      style={S.chordActionBtn}
                    >🎲</button>
                    <button
                      onClick={(e) => { e.stopPropagation(); rejectChord(chord.name); }}
                      title={`Reject "${chord.name}" (won't be suggested again)`}
                      style={{ ...S.chordActionBtn, right: 6, left: "auto" }}
                    >✕</button>

                    <div onClick={() => previewChord(chord)} style={{ cursor: "pointer" }}>
                      <div style={{ fontSize: 11, fontFamily: "monospace", letterSpacing: 2, marginBottom: 8, color: col.hex }}>
                        {chord.romanNumeral}
                      </div>
                      <div style={{ fontSize: "1.6rem", color: "#f1f5f9", marginBottom: 6 }}>{chord.name}</div>
                      <div style={{ fontSize: 10, color: "#475569", fontFamily: "monospace" }}>{chord.function}</div>
                      <div style={{ fontSize: 10, color: "#334155", marginTop: 6, fontFamily: "monospace" }}>
                        {(chord.notes || []).join(" ")}
                      </div>
                      <div style={{ fontSize: 9, color: "#1e293b", marginTop: 4, fontFamily: "monospace" }}>
                        tap to preview
                      </div>
                    </div>
                  </div>
            ...
            {mutatingSlot >= 0 && (
              <div style={S.explorePanel}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ ...S.vcLabel, marginBottom: 0, color: theme.accent2 }}>
                    🎲 Alternatives for chord #{mutatingSlot + 1}
                  </div>
                  <button onClick={() => { setMutatingSlot(-1); setMutations([]); }} style={S.tinyCloseBtn}>×</button>
                </div>
                {loadingMutations && (
                  ...
                  <div style={S.mutationsGrid}>
                    {mutations.map((m, idx) => (
                      <div key={idx} style={S.mutationCard}>
                        <div style={{ fontSize: 10, fontFamily: "monospace", color: theme.accent, marginBottom: 4 }}>
                          {m.romanNumeral}
                        </div>
                        <div style={{ fontSize: "1.2rem", color: "#f1f5f9", marginBottom: 8 }}>{m.name}</div>
                        <div style={{ display: "flex", gap: 6 }}>
                          <button
                            onClick={async () => { await ensureAudio(); playChordTone(m.notes, 1.5, activeTone); }}
                            style={S.miniBtn}
                          >▶</button>
                          <button onClick={() => applyMutation(m)} style={{ ...S.miniBtn, background: theme.accent, color: "#fff" }}>
                            Use
                          </button>
                        </div>
                      </div>
            ...
            <div style={S.vibeControls}>
              <div style={S.vcLabel}>🎛 Vibe Controls — tweak before playing</div>
              <SliderRow label="TEMPO" value={bpm} min={40} max={200} onChange={setBpm} display={`${bpm} bpm`} />
              <SliderRow label="REVERB" value={reverb} min={0} max={100} onChange={setReverb} display={`${reverb}%`} />
              <SliderRow label="SUSTAIN" value={decay} min={10} max={100} onChange={setDecay} display={`${decay}%`} />
              <SliderRow label="ARPEGGIATE" value={arpeggiate} min={0} max={100} onChange={setArpeggiate} display={arpeggiate === 0 ? "off" : `${arpeggiate}%`} />
              <div style={S.vcRow}>
                <span style={S.vcName}>TONE</span>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button
                    className={activeTone === "piano" ? "sound-btn active" : "sound-btn"}
                  ...
                  {[
                    ...
                    <button
                      ...
                      onClick={() => openProModal("Unlock the full sound palette", "Rhodes, Pad, and Pluck tones are coming with Pro.")}
                    >
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
              ...
              <button className="btn btn-share" onClick={openShare}>🔗 Share</button>
              <button className="btn btn-record" onClick={exportAudio}>⏺ Export Audio</button>
              <button className="btn btn-secondary" onClick={exportMidi}>🎵 Export MIDI</button>
            </div>
            ...
            <div style={S.infoGrid} className="info-grid-responsive">
              <div style={S.infoCard}>
                ...
                <div style={{ color: "#94a3b8", fontSize: 14, fontStyle: "italic", lineHeight: 1.6 }}>
                  "{result.vibe}"
                </div>
              </div>
              <div style={S.infoCard}>
                <div style={S.infoCardLabel}>Chord Sequence</div>
                <div style={{ color: "#f1f5f9", fontSize: 18, letterSpacing: 4, fontFamily: "monospace" }}>
            ...
            <div style={{ ...S.infoCard, marginBottom: 16 }}>
              ...
              <div style={{ color: "#94a3b8", fontSize: 14, lineHeight: 1.7 }}>{result.theory}</div>
            </div>
            ...
            <div style={{ ...S.explorePanel, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
                ...
                <button
                  ...
                  style={{ ...S.exploreBtn, opacity: loadingVariations ? 0.5 : 1 }}
                >
                  {loadingVariations ? "Generating..." : (variations.length ? "🔄 More variations" : "🎨 Show 4 variations")}
                </button>
              </div>
              ...
              {rejected.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, fontFamily: "monospace", letterSpacing: 2, color: "#64748b", marginBottom: 6 }}>
                ...
                <div style={S.variationsGrid}>
                  {variations.map((v, idx) => (
                    <div key={idx} style={S.variationCard}>
                      <div style={{ fontSize: 11, fontFamily: "monospace", letterSpacing: 2, color: theme.accent, marginBottom: 4 }}>
                        VARIATION {idx + 1}
                      </div>
                      <div style={{ fontSize: 15, color: "#f1f5f9", fontStyle: "italic", marginBottom: 6 }}>
                        {v.title}
                      </div>
                      <div style={{ fontSize: 11, color: "#94a3b8", fontFamily: "monospace", marginBottom: 10, letterSpacing: 1 }}>
                        {v.chords.map((c) => c.name).join(" – ")}
                      </div>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        <button onClick={() => previewProgression(v)} style={S.miniBtn}>▶ Preview</button>
                        <button onClick={() => applyVariation(v)} style={{ ...S.miniBtn, background: theme.accent, color: "#fff" }}>
                          Use this
                        </button>
                      </div>
                    </div>
              ...
              {!variations.length && !loadingVariations && (
                <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.6 }}>
                  Tap <strong style={{ color: theme.accent2 }}>🎨 Show 4 variations</strong> to get fresh takes on this vibe.
                  Or use 🎲 on a chord to swap it for something unexpected. Hit ✕ on chords you don't want to see again.
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {history.slice(1).map((h, i) => (
                <button
                  ...
                  onClick={() => { setResult(h.result); setIsExample(false); setPrompt(h.prompt); if (h.result.bpm) setBpm(h.result.bpm); }}
                >
                  <span>{h.prompt}</span>
  ...
  return (
    <div style={S.vcRow}>
      ...
      <input
        ...
        style={{ flex: 1, minWidth: 100, accentColor: "#6366f1", cursor: "pointer" }}
      />
      <span style={{ fontSize: 12, color: "#94a3b8", fontFamily: "monospace", width: 48, textAlign: "right" }}>
        {display}
      </span>
    </div>
  );
...
function EmailCapture({ source, accent }) {
  ...
  const submit = () => {
    ...
    setTimeout(() => { setBtnText("Notify Me"); setStatus(""); }, 3000);
  };
    ...
    <div style={{ textAlign: "center", marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <input
          ...
          style={S.emailInput}
        />
        <button onClick={submit} style={{ ...S.emailBtn, background: accent }}>{btnText}</button>
      </div>
      <div style={{ fontSize: 11, color: statusColor, marginTop: 8, minHeight: 14 }}>{status}</div>
    </div>
  );
}
...
const S = {
  body: { minHeight: "100vh", background: "#000", fontFamily: "'Georgia',serif", color: "#e2e8f0", overflowX: "hidden", position: "relative" },
  ambient: { position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none",
    background: "radial-gradient(ellipse 60% 40% at 15% 10%, rgba(50,15,100,0.04) 0%, transparent 50%), radial-gradient(ellipse 45% 35% at 85% 85%, rgba(8,50,75,0.035) 0%, transparent 50%)" },
  notesBg: { position: "fixed", inset: 0, zIndex: 0, pointerEvents: "none", overflow: "hidden" },
  app: { position: "relative", zIndex: 1, maxWidth: 800, margin: "0 auto", padding: "48px 24px 100px" },
  topBar: { position: "absolute", top: 20, right: 24, display: "flex", gap: 10, alignItems: "center", zIndex: 5 },
  proBtn: { background: "linear-gradient(135deg, rgba(99,102,241,0.25) 0%, rgba(167,139,250,0.25) 100%)", border: "1px solid rgba(167,139,250,0.5)", borderRadius: 24, color: "#e0e7ff", padding: "10px 22px", fontSize: 15, fontWeight: 600, cursor: "pointer", fontFamily: "Georgia,serif", boxShadow: "0 2px 12px rgba(167,139,250,0.2)" },
  settingsBtn: { position: "absolute", top: 20, left: 24, zIndex: 5, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: "50%", width: 38, height: 38, color: "#94a3b8", fontSize: 17, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, lineHeight: 1 },
  engLabel: { fontSize: 11, letterSpacing: 6, color: "#6366f1", textTransform: "uppercase", marginBottom: 16, fontFamily: "monospace", textAlign: "center" },
  h1: { fontSize: "clamp(1.8rem,5vw,3.2rem)", fontWeight: 400, lineHeight: 1.1, color: "#f1f5f9", letterSpacing: "-0.02em", textAlign: "center", marginBottom: 14 },
  subtitle: { color: "#64748b", fontSize: 15, lineHeight: 1.6, textAlign: "center", marginBottom: 8 },
  sampleStatus: { textAlign: "center", fontSize: 11, fontFamily: "monospace", letterSpacing: 2, marginBottom: 24, transition: "color .3s" },
  inputWrap: { position: "relative", marginBottom: 14 },
  textarea: { width: "100%", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, padding: "18px 110px 18px 20px", color: "#f1f5f9", fontSize: 16, fontFamily: "Georgia,serif", resize: "none", outline: "none", lineHeight: 1.5, boxSizing: "border-box" },
  generateBtn: { position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", border: "none", borderRadius: 8, color: "#fff", padding: "10px 18px", fontSize: 14, fontFamily: "monospace", letterSpacing: 1 },
  errorBox: { background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.2)", borderRadius: 10, padding: "14px 18px", color: "#fca5a5", marginBottom: 24, fontSize: 14 },
  resultLabel: { fontSize: 11, fontFamily: "monospace", letterSpacing: 4, color: "#6366f1", textTransform: "uppercase", marginBottom: 8 },
  exampleBanner: { display: "flex", alignItems: "center", gap: 10, background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.22)", borderRadius: 10, padding: "12px 16px", marginBottom: 20, fontSize: 13, color: "#94a3b8", lineHeight: 1.5 },
  resultTitle: { fontSize: "1.8rem", fontWeight: 400, color: "#f1f5f9", fontStyle: "italic", marginBottom: 8 },
  metaTag: { fontSize: 12, color: "#64748b", fontFamily: "monospace", letterSpacing: 1 },
  grooveBadge: { fontSize: 11, color: "#a78bfa", fontFamily: "monospace", letterSpacing: 1, background: "rgba(167,139,250,0.12)", border: "1px solid rgba(167,139,250,0.3)", borderRadius: 12, padding: "3px 10px", textTransform: "capitalize" },
  pianoWrap: { marginBottom: 24, overflowX: "auto", background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 12, padding: 16 },
  chordGrid: { display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 },
  vibeControls: { background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 12, padding: "20px 24px", marginBottom: 20 },
  vcLabel: { fontSize: 10, fontFamily: "monospace", letterSpacing: 3, color: "#6366f1", textTransform: "uppercase", marginBottom: 16 },
  vcRow: { display: "flex", alignItems: "center", gap: 14, marginBottom: 12, flexWrap: "wrap" },
  vcName: { fontSize: 12, color: "#64748b", fontFamily: "monospace", width: 76, flexShrink: 0 },
  infoGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 },
  infoCard: { background: "rgba(255,255,255,.03)", border: "1px solid rgba(255,255,255,.06)", borderRadius: 10, padding: "18px 20px" },
  chordActionBtn: { position: "absolute", top: 6, left: 6, width: 22, height: 22, borderRadius: "50%", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(255,255,255,0.1)", color: "#94a3b8", fontSize: 11, cursor: "pointer", padding: 0, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, zIndex: 2 },
  explorePanel: { background: "linear-gradient(135deg, rgba(167,139,250,0.06) 0%, rgba(99,102,241,0.03) 100%)", border: "1px solid rgba(167,139,250,0.18)", borderRadius: 12, padding: "18px 20px", marginBottom: 20 },
  exploreBtn: { background: "linear-gradient(135deg, rgba(167,139,250,0.25) 0%, rgba(99,102,241,0.25) 100%)", border: "1px solid rgba(167,139,250,0.4)", borderRadius: 20, color: "#e0e7ff", padding: "8px 16px", fontSize: 12, cursor: "pointer", fontFamily: "Georgia,serif", whiteSpace: "nowrap" },
  rejectedPill: { background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", borderRadius: 14, color: "#fca5a5", padding: "4px 10px", fontSize: 11, fontFamily: "monospace", cursor: "pointer" },
  variationsGrid: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 },
  variationCard: { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "14px 14px 12px" },
  mutationsGrid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 },
  mutationCard: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "12px 10px", textAlign: "center" },
  miniBtn: { background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, color: "#cbd5e1", padding: "5px 10px", fontSize: 11, cursor: "pointer", fontFamily: "Georgia,serif", flex: 1 },
  tinyCloseBtn: { background: "transparent", border: "none", color: "#64748b", fontSize: 18, cursor: "pointer", padding: "0 4px", lineHeight: 1 },
  infoCardLabel: { fontSize: 10, fontFamily: "monospace", letterSpacing: 3, color: "#6366f1", textTransform: "uppercase", marginBottom: 10 },
  historyLabel: { fontSize: 10, fontFamily: "monospace", letterSpacing: 4, color: "#334155", textTransform: "uppercase", marginBottom: 14 },
  modalOverlay: { position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 },
  modalBox: { background: "linear-gradient(180deg, #0f0f1a 0%, #0a0a0f 100%)", border: "1px solid", borderRadius: 16, padding: "36px 32px", width: "100%", position: "relative", boxShadow: "0 0 60px rgba(99,102,241,0.15)" },
  modalClose: { position: "absolute", top: 12, right: 16, background: "none", border: "none", color: "#64748b", fontSize: 24, cursor: "pointer" },
  modalTitle: { fontSize: "1.4rem", fontWeight: 400, color: "#f1f5f9", fontStyle: "italic", marginBottom: 10 },
  kicker: { fontSize: 11, fontFamily: "monospace", letterSpacing: 4, color: "#6366f1", textTransform: "uppercase", marginBottom: 10 },
  featureBox: { background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 20px", marginBottom: 20 },
  featureLabel: { fontSize: 11, fontFamily: "monospace", letterSpacing: 3, color: "#6366f1", marginBottom: 12 },
  featureList: { color: "#cbd5e1", fontSize: 13, lineHeight: 1.9 },
  ghostBtn: { width: "100%", background: "transparent", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#94a3b8", padding: 12, fontSize: 14, cursor: "pointer", fontFamily: "Georgia,serif" },
  primaryBtn: { background: "rgba(99,102,241,0.9)", border: "none", borderRadius: 8, color: "#fff", padding: 14, fontSize: 14, cursor: "pointer", fontFamily: "Georgia,serif" },
  platformBtn: { background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#94a3b8", padding: 10, fontSize: 13, cursor: "pointer", fontFamily: "Georgia,serif" },
  urlBox: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: 14, marginBottom: 16, wordBreak: "break-all", fontFamily: "monospace", fontSize: 12, color: "#94a3b8", maxHeight: 120, overflowY: "auto" },
  emailInput: { flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "12px 14px", color: "#f1f5f9", fontSize: 14, fontFamily: "Georgia,serif", outline: "none" },
  emailBtn: { border: "none", borderRadius: 8, color: "#fff", padding: "12px 18px", fontSize: 13, cursor: "pointer", fontFamily: "Georgia,serif", whiteSpace: "nowrap" },
  pricingGrid: { display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 4 },
  priceCard: { borderRadius: 12, padding: "20px 14px 16px", textAlign: "center", transition: "all .2s" },
  bestValueBadge: { position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: "#a78bfa", color: "#0a0a0f", fontSize: 9, fontWeight: 700, letterSpacing: 1, padding: "3px 10px", borderRadius: 10, fontFamily: "monospace", whiteSpace: "nowrap" },
  priceLabel: { fontSize: 11, fontFamily: "monospace", letterSpacing: 2, color: "#94a3b8", textTransform: "uppercase", marginBottom: 12 },
  priceAmount: { fontSize: 32, fontWeight: 600, color: "#f1f5f9", lineHeight: 1, marginBottom: 8 },
  priceNote: { fontSize: 11, color: "#a78bfa", marginBottom: 14, lineHeight: 1.4, minHeight: 28 },
  priceCta: { width: "100%", border: "none", borderRadius: 8, color: "#fff", padding: "10px 8px", fontSize: 13, cursor: "pointer", fontFamily: "Georgia,serif", transition: "filter .15s" },
};
